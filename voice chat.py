    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                continue
time.sleep(1)
community = client.sub_clients(start=0, size=10000)
...
zefil="""
заходи в телеграм канал по скриптам!
t.me/zefilCantor
"""
...
castel = []
users = sub_client.get_all_users(start=0, size=10000)
a = 1
...
try:
    ...
    pass
print('чат создан.')
chat = sub_client.get_chat_threads(size=1000)
for number_chat, title in enumerate(chat.title, 1):
 print(f"{number_chat}.{title}")
chatid = chat.chatId[int(input("выберите чат: "))-1]
sub_client.edit_chat(viewOnly=True,
                     chatId=chatid,
                     title='@zefilCantor',
                     content=zefil)
def zefil():
    try:
        sub_client.invite_to_vc(userId=id,
                                  chatId=chatid)
    except:
        pass
print('приглашаю онлайн участников.')
for i in range(0, 5000, 1000):
    users = sub_client.get_online_users(start=i, size= 10000)
    if users:
        for name, id, level, status in zip(users.profile.nickname,
                                           users.profile.userId,
                                           users.profile.level,
...
for i in range(0, 10000, 1000):
    users = sub_client.get_online_users(start=i, size= 100000)
    if users:
        for name, id, level, status in zip(users.profile.nickname,
                                           users.profile.userId,
                                           users.profile.level,
